...
/* Initialize MPI */
		...
		npcols= dims[1];
		period[0]=period[1]=0;
		MPI_Cart_create (MPI_COMM_WORLD, 2, dims, period, 1, &new_comm);
...
/* Read input on process 0 and distribute to processes */
	if (rank==0){
        	...
        	fp = fopen("/home/c1544657/Desktop/hpc2/David.ps", "r");
		while(! feof(fp))
		{
			icheck = fscanf(fp, "\n%[^\n]", str);
...
/* Gather data from processes and output on process 0 */
	if(rank==0){
		localrow = 1;
		fout= fopen("/home/c1544657/Desktop/hpc2/DavidBlur.ps", "w");
